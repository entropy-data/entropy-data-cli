...
- Publish keywords and `Issues`/`Changelog` links in the PyPI package metadata so the project is findable and its sidebar links to the tracker and the changelog.
...
- The `semantics` graph is copied as one OSI-compliant ontology YAML document per namespace (`semantic-ontology/<namespace>.yaml`) via the app's `GET`/`PUT /api/semantics/experimental/namespaces/{ns}/ontology.yaml` endpoint, which imports it in the correct internal order (groups before members, concepts before relationships) and provisions the namespace from the metadata (`display_name`, `read_only`, `owner`) carried in the document's root `custom_properties`. There is no separate namespace artifact; a tree from an older version that still carries a `semantic-namespaces/` folder is applied as before. Requires that endpoint on the target instance.
...
- Update dependencies to their latest versions to clear all open security advisories: `urllib3` 2.7.0 (cross-origin header forwarding on proxied redirects, decompression-bomb bypass), `requests` 2.34.2 (insecure temp file reuse in `extract_zipped_paths()`), `idna` 3.18 (`idna.encode()` bypass of the CVE-2024-3651 fix), `pygments` 2.20.0 (ReDoS in GUID matching), and `pytest` 9.1.1 (vulnerable tmpdir handling). The minimum `requests` is now 2.33 so installs from PyPI also get the fix.
